...
import { getRatingDataById } from "@/server/services/rating/getRatindDataById";
import { TNewRating, TUpdateRateData } from "@/server/controllers/rating/types";
import {
  createValidationSchema,
  updateValidationSchema,
} from "@/server/controllers/rating/validationSchema";
import { updateRate } from "@/server/services/rating/updateRate";
import { createNewRating } from "@/server/services/rating/createNewRating";
import { getRatingDataByPostId } from "@/server/services/rating/getRatingByPostId";
import { revalidatePath } from "next/cache";
import { getPostById } from "@/server/services/post/getPostById";
...
export const updateRating = async ({
  ...
  if (post) {
    ...
    if (subCategoryId) {
      revalidatePath(
        `/${locale}${categoryId ? "/categories/" + categoryId : ""}/subCategory/${subCategoryId}`,
      );
    } else {
